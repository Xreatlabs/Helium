...
async function toggleBackupSystem(checkbox) {
  ...
  try {
    ...
    alert('❌ Failed to update backup system setting');
    console.error(error);
    checkbox.checked = !enabled; // Revert
  }
}
...
async function toggleAutomaticBackups(checkbox) {
  ...
  try {
    ...
    if (data.success) {
      alert(`✅ Automatic backups ${enabled ? 'enabled' : 'disabled'}!\n\n⚠️ Please restart the server for changes to take effect.`);
      await loadBackups();
    } else {
      alert('❌ Error: ' + data.error);
      checkbox.checked = !enabled; // Revert
    ...
    alert('❌ Failed to update automatic backup setting');
    console.error(error);
    checkbox.checked = !enabled; // Revert
  }
}
