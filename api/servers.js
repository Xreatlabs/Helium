...
module.exports.load = async function (app, db) {
  app.get("/updateinfo", async (req, res) => {
    ...
    if (!cacheaccount || !cacheaccount.data || !cacheaccount.data.attributes) {
      if (!res.headersSent) {
  ...
  app.get("/create", async (req, res) => {
    ...
    if (newsettings.api.client.allow.server.create == true) {
      queue.addJob(async (cb) => {
        ...
        if (!cacheaccount || !cacheaccount.data || !cacheaccount.data.attributes) {
          console.error('[Server Create] Missing cacheaccount or cacheaccount.data.attributes');
          if (!res.headersSent) {
            cb();
            return res.send(
        ...
        if (
          ...
          if (!isNaN(ram) && !isNaN(disk) && !isNaN(cpu)) {
            ...
            // Check against available resources (base + purchased)
